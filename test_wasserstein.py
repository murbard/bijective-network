...
class Network(object):
    ...
    def __init__(self, dim, n_s_layers, n_w_layers, batch_size = 1)
        self.dim = dim
        self.s_layers = [
            Layer(dim, relu=1.0 if i < n_s_layers - 1 else None) for i in range(n_s_layers)]
        self.w_layers = [
            Layer(2 * dim, relu=1.0 if i  < n_w_layers - 1 else None) for i in range(n_w_layers)]
        self.batch_size = batch_size

    def forward(real):
        noise = np.randon.randn(self.batch_size, self.dim)
        fake = generate(inverse(real) + noise)
        joint = np.concatenate([fake, real], axis=1)
        p = joint_probability(joint)
        cost = ((real - fake)**2).sum() * p /
